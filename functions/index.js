...
// Price ID mensile - da configurare in Stripe Dashboard
// Puoi impostare il valore qui o configurarlo dopo il deploy
const stripeMonthlyPriceId = defineString("STRIPE_PRICE_MONTHLY_ID", {
  default: "price_1SoKa8IXTFeCafw3lTiATDrx", // PRO Mensile €19,99
});
// Price ID annuale - da configurare in Stripe Dashboard
// Puoi impostare il valore qui o configurarlo dopo il deploy
const stripeYearlyPriceId = defineString("STRIPE_PRICE_YEARLY_ID", {
  default: "price_1SoKaQIXTFeCafw3QV4tUsWD", // PRO Annuale €119,99
});
const stripeWebhookSecret = defineSecret("STRIPE_WEBHOOK_SECRET");
const appUrl = defineString("APP_URL", {
  default: "https://cutcalendar-408e5.web.app",
});
...
// Crea sessione checkout per pagamento servizio singolo
exports.createServiceCheckoutSession = functions
    .runWith({secrets: [stripeSecretKey]})
    .https
    .onCall(async (data, context) => {
      // Verifica autenticazione
      ...
      const uid = context.auth.uid;
      const email = context.auth.token.email || data.userEmail;
      const {bookingId, amount, bookingData} = data;
      ...
      try {
        ...
        // Crea sessione checkout per pagamento una tantum
        const session = await stripe.checkout.sessions.create({
          customer: customerId,
          mode: "payment",
          payment_method_types: ["card"],
          line_items: [
            {
              price_data: {
                currency: "eur",
                product_data: {
                  name: bookingData?.service || "Servizio",
          ...
          success_url: `${baseUrl}/admin.html?` +
          `session_id={CHECKOUT_SESSION_ID}&booking_id=${bookingId}&success=true`,
          cancel_url: `${baseUrl}/admin.html?success=false`,
          metadata: {
            firebaseUID: uid,
            bookingId: bookingId,
        ...
        console.log(
            `[ServiceCheckout] Sessione creata: ${session.id} per booking ${bookingId}`);
        return {url: session.url, sessionId: session.id};
      } catch (error) {
        console.error("[ServiceCheckout] Errore:", error);
        throw new functions.https.HttpsError(
            "internal",
            `Errore nella creazione del checkout servizio: ${error.message}`);
      }
    });
...
async function handleCheckoutCompleted(session, db, stripeInstance) {
  ...
  // Gestisci pagamento servizio singolo
  if (session.metadata.type === "service_payment") {
    ...
    if (bookingId && session.payment_status === "paid") {
      try {
        ...
        console.log(`Pagamento servizio completato per booking ${bookingId}`);
      } catch (error) {
