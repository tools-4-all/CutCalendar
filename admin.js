...
async function generatePDFReport(bookings, totalRevenue, totalRevenueConfirmed, statusCounts, serviceStats, startDate, endDate) {
    try {
        ...
        for (let i = 1; i <= pageCount; i++) {
            ...
            doc.text('CutCalendar - Sistema di Gestione Prenotazioni', 105, 290, { align: 'center' });
        }
...
function generateCSVReport(bookings, totalRevenue, totalRevenueConfirmed, statusCounts, serviceStats, startDate, endDate) {
    try {
        ...
        csvLines.push('Data,Cliente,Email,Telefono,Servizio,Prezzo,Stato,Metodo Pagamento');
        bookings
            .sort((a, b) => {
                const dateA = a.data().dateTime?.toDate() || new Date(0);
                const dateB = b.data().dateTime?.toDate() || new Date(0);
...
function shareViaWhatsApp() {
    ...
    const message = `💇‍♀️ Ciao! Prenota un appuntamento presso ${companyName}:\n\n${link}\n\nPrenotazione rapida e semplice! ✂️💅`;
...
function downloadQRCode() {
    ...
    try {
        if (canvas) {
            ...
            // Se c'è un'immagine, scaricala direttamente
            const link = document.createElement('a');
            link.href = img.src;
            link.download = `qr-code-prenotazioni-${new Date().toISOString().split('T')[0]}.png`;
            document.body.appendChild(link);
